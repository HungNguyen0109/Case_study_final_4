...
public interface IInOutRepo extends CrudRepository<InOut, Long> {
    ...
    @Query(nativeQuery = true, value = "select sum(money) as inFlow from add_money where wallet_id " +
            "in (select wallet_id from wallets where user_id = ?)  and Month(date) = ? and Year(date) = ?;")
    Integer getMonthInFlow(Long idUser, int month, int year);



    @Query(nativeQuery = true, value = "select sum(amount) as outFlow from transactions where wallet_id " +
            "in (select wallet_id from wallets where user_id = ?)  and Month(date) = ? and Year(date) = ?;")
    Integer getMonthOutFlow(Long idUser, int month, int year);
    ...
    Integer getIncomeTotal(Long idWallet);
